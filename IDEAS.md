...
- 2026-09-03 — `/you`, `:device` state, kept_count == 0 on a bridge-capable shell: "Nothing is kept on this phone yet." gives no lead-in before the Google/Apple sign-in doors appear right below — unlike the kept_count > 0 + doors branch, which now says "Sign in to save your kept work(s) to your account" (decisions/0025). Pre-existing, not introduced by today's changes; surfaced by a `/simplify` review pass. Untested.
