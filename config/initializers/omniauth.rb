...
Rails.application.config.middleware.use OmniAuth::Builder do
  ...
  provider :apple,
    Rails.application.credentials.dig(:apple, :client_id).presence || ENV["APPLE_CLIENT_ID"],
    "",
    scope: "email name",
    team_id: Rails.application.credentials.dig(:apple, :team_id).presence || ENV["APPLE_TEAM_ID"],
    key_id: Rails.application.credentials.dig(:apple, :key_id).presence || ENV["APPLE_KEY_ID"],
    pem: Rails.application.credentials.dig(:apple, :private_key).presence || ENV["APPLE_PRIVATE_KEY"]
end
